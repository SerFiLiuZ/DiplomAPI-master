...
func (r *DBControllerRepository) AcceptApplication(chatIDUser int) error {
	_, err := r.store.db.Exec(`
        UPDATE taskdb.applications 
        SET done = 1 
        WHERE chatID =? AND done = 0;
    `, chatIDUser)
	return err
}

func (r *DBControllerRepository) RejectApplication(chatIDUser int, IDManager int) error {
	_, err := r.store.db.Exec(`
        UPDATE taskdb.applications 
        SET done = 1 
        WHERE chatID =? AND done = 0;
