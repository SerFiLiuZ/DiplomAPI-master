...
func (r *CardRepository) FindCardsByBoardID(IDUser int, BoardID int) ([]*model.Card, error) {
	...
	rows, err := r.store.db.Query("SELECT idCard, name, description, done FROM taskdb.cards WHERE idBoard = ? AND idBoard IN (SELECT idBoard FROM taskdb.boards WHERE idUser = ?)", BoardID, IDUser)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
