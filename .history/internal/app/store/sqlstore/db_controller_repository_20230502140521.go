...
func (r *DBControllerRepository) SendMesage(TaskTitle string, TaskDueDate string, TaskSelectedWorkers []*model.User) error {
	rows, err := r.store.db.Query(
		"SELECT idTask, idCard, idManager, title, dueDate, selectedWorkers FROM taskdb.tasks WHERE title =? AND dueDate =? AND selectedWorkers =?",
		TaskTitle,
		TaskDueDate,
		TaskSelectedWorkers,
	)
	...
	for rows.Next() {
		task := &model.Task{}
		err = rows.Scan(&task.IDTask, &task.IDCard, &task.IDManager, &task.Title, &task.DueDate, &task.SelectedWorkers)
		if err != nil {
			return err
		}
	}
