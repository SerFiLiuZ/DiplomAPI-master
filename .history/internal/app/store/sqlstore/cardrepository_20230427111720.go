...
func (r *CardRepository) FindCardsByBoardID(IDUser int, BoardID int) ([]*model.Card, error) {
	// Execute the SQL query
	rows, err := r.store.db.Query("SELECT idCards, name, description, done FROM taskdb.cards WHERE idBoard = ? AND idBoard IN (SELECT idBoard FROM taskdb.boards WHERE idUser = ?)", BoardID, IDUser)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
