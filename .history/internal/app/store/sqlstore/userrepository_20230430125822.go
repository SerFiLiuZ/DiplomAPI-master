...
func (r *UserRepository) Find(id int) (*model.User, error) {
	...
	if err := r.store.db.QueryRow(
		"SELECT idUsers, FIO, status, idLegal_entity, idTgInfo, email, password FROM users WHERE idUsers = ?",
		id,
	).Scan(
		&u.ID,
		&u.FIO,
...
func (r *UserRepository) FindAllById(AssignedIdUser string) (string, error) {
	...
	log.Print(AssignedFIOUser)
	if len(AssignedFIOUser) < 2 {
		return "", store.ErrRecordNotFound
	}
	return AssignedFIOUser[2:], store.ErrRecordNotFound
