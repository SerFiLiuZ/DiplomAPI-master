...
func (r *TaskRepository) FindTasksByCardID(IDUser int, TaskId int) ([]*model.Task, error) {
...
func (r *TaskRepository) CreateTask(TaskTitle string, TaskDes string, TaskDueDate string, TaskSelectedWorkers []*model.User, CardId int) error {
	_, err := r.store.db.Exec(
		"INSERT INTO taskdb.tasks (name, description, due_date, assigned, card_id) VALUES (?,?,?,?,?)",
		TaskTitle,
		TaskDes,
		TaskDueDate,
		false,
