...
func (r *TaskRepository) FindTasksByCardID(IDUser int, UserStatus string, CardId int) ([]*model.Task, error) {
	...
	if UserStatus == "manager" {
		...
		FROM taskdb.tasks JOIN taskdb.cards ON taskdb.tasks.idCard = taskdb.cards.idCard 
		JOIN taskdb.boards ON taskdb.cards.idBoard = taskdb.boards.idBoard	
	...
	if UserStatus == "worker" {
		response = `
		SELECT taskdb.tasks.idTask, taskdb.tasks.name, taskdb.tasks.description, taskdb.tasks.due_date, taskdb.tasks.done, taskdb.tasks.assigned
		FROM taskdb.tasks WHERE FIND_IN_SET(?, assigned) > 0 AND idCard = ?;
		`
	}
