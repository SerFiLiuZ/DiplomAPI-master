...
func (r *BoardRepository) InsertBoardToDeleteBoard(IDBoard int) error {
	b, err := r.GetBoard(IDBoard)
	if err != nil {
		log.Println(err)
		return err
	}
...
func (r *BoardRepository) DeleteBoard(IDBoard int) error {
	log.Print("Удаляю доску")
	log.Print("Переношу доску")
	err := r.InsertBoardToDeleteBoard(IDBoard)
	if err != nil {
		log.Println(err)
		return err
	}

	log.Print("Перенёс доску")
	log.Print("Беру карты")
	c, err := r.store.cardRepository.FindCardsByBoardIDSimple(IDBoard)
	if err != nil {
		log.Println(err)
		return err
	}
	log.Print("Взял карты")
	for _, card := range c {
		err := r.store.cardRepository.DeleteCard(card.ID)
		if err != nil {
			log.Println(err)
			return err
		}
