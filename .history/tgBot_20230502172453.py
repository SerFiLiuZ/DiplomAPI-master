...
def checkPasswordOrg(p):
    ...
    mycursor.execute("SELECT COUNT(*) FROM users WHERE passwordCorp = %s AND users.status = 0", (p,))
...
def getNameOrg(password):
    ...
    mycursor = mydb.cursor()
    # Execute the SQL query
    mycursor.execute("SELECT nameCorp FROM users WHERE passwordCorp=%s LIMIT 1", (password,))
