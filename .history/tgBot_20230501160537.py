...
def checkPasswordOrg(p):
    ...
    mycursor.execute(f"SELECT COUNT(*) FROM taskdb.users WHERE passwordCorp={p} and taskdb.users.status = 0")
    print("Checking password2")
    result = mycursor.fetchone()[0]
    print("Checking password3")
    # Commit the changes to the database
    mydb.commit()
    print("Checking password4")
    if result > 0:
        return True
    else:
        return False
...
def getNameOrg(password):
    ...
    # Execute the SQL query
    mycursor.execute("SELECT COUNT(*) FROM taskdb.users WHERE passwordCorp='%s' and taskdb.users.status = 0", (password,))
